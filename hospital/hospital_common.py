...
import xlrd
import xlsxwriter as xlsxw  # read and write xls(x) files
...
def xls2sqlite(xlsfile, dbfile, table=None):
    ...
    # if table is not defined, get it from xlsfile
    table = table or tfunc(xlsfile)

    # initialize workbook and get first sheet
    wb = xlrd.open_workbook(xlsfile)  # ,encoding_override="cp1252")#"latin-1")
    sh = wb.sheet_by_index(0)

    # infer types for database creation for each column, based on the most
    # common type
    ...
    # render create statement from the infered types using first row as column
    # names
    colld = {}  # collision dict for duplicate collumns
    header = ['"%s"' % nfunc(i, v, collision_dict=colld)
              for i, v in enumerate(sh.row_values(0))]
    definition = ['%s %s' % (n,
                             XLRD_TYPES[types[i].index(max(types[i]))]) for i, n in enumerate(header)]
    create = CREATE % (table, ','.join(definition))
    insert = INSERT.format(table, ','.join(
        ['"primary_id"'] + header).replace('%', '%%'))

    dbfile.execute(create)  # execute create

    # For each row insert them as strings, taking advantage of sqlite text
    # based type sistem
    for nrow in range(1, sh.nrows):
...
def parseCustomFile(fname, gbeg, gheaders, gtypes, conn, basetable=None):
    ...
    gschema = CREATE % (gtablename, ','.join(
        [u'"%s" TEXT' % nfunc(i, s) for i, s in enumerate(gbeg)]))
    dschema = CREATE % (dtablename, ','.join([u'"%s" %s' % (nfunc(i, s), gtypes[i]) for i, s in enumerate(
        gheaders)] + [u'"group_ref" INTEGER REFERENCES "%s"("primary_id")' % gtablename]))

    ginsert = INSERTNODEF.format(gtablename)
    dinsert = INSERT.format(dtablename, ','.join(
        ['"%s"' % nfunc(i, v) for i, v in enumerate(gheaders)] + ['"group_ref"']))
    ...
    reheader = re.compile(''.join(
        [r'\s*%s:\s*(.+)' % s for s in gbeg]), re.U | re.I)  # header match regex
    ngroup = 0
    newgroup = False
    offsets = None
    grouprow = None
    ...
    with codecs.open(fname, "rb", "latin-1") as f:
        for line in f:  # process the file line-wise
            ...
            # try to match group header with regex
            ...
            if header is not None:  # it is a header
                ...
                insert = dinsert % ','.join(
                    ["'%s'" % d for d in data] + [unicode(ngroup)])
                conn.execute(insert)
...
def writecxlsfromsqlite(output, c, colstyle={}, formula_cells={}, log=print):
    ...
    headers = [urllib.unquote(i.replace("_", " ")).decode(
        'utf8').upper() for i in description]
        ...
        for j, v in enumerate(l):
            ...
            if description[j] in formula_cells:
                ...
                worksheet.write_formula(
                    i + 1, j, f["formula"].format(*paramsi), value=v, cell_format=style)

            # if it is a normat cell
            else:
                worksheet.write(i + 1, j, v, style)
